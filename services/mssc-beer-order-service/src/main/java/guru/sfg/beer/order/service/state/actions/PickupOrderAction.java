...
import guru.sfg.brewery.model.events.PickupOrderRequest;
...
public class PickupOrderAction implements Action<BeerOrderStatusEnum, BeerOrderEventEnum> {
  ...
  public void execute(StateContext<BeerOrderStatusEnum, BeerOrderEventEnum> context) {
    ...
    repository.findById(UUID.fromString(beerOrderId)).ifPresent(beerOrder -> {
      PickupOrderRequest request = PickupOrderRequest.builder()
          .beerOrder(mapper.beerOrderToDto(beerOrder))
          .build();
      jmsTemplate.convertAndSend(JmsConfig.PICKUP_ORDER_QUEUE, request);
    });
  }
